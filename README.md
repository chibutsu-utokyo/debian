...
### インストール済みソフトウェア
このcodespaceのベースイメージは `mcr.microsoft.com/devcontainers/python:1-3.12-bookworm` で，Debian Bookworm ベースに Python 3.12 がインストールされたものです．

加えて以下のソフトウェアがインストールされています：

- C/C++，Fortranコンパイラ，gitなどの開発ツール
- Python関連の主要パッケージ（JupyterLab，Jupyter Notebook，NumPy，SciPy，pandas，matplotlib，numba，marimo など）
- `uv` による固定版のPythonパッケージ管理
- Ruff（`ruff` コマンド）
- GitHub Copilot（VS Code 拡張機能）

詳細は `.devcontainer` ディレクトリ以下の設定ファイルを参照してください．

### AIコーディングアシスタントのインストール（任意）
以下のAIコーディングアシスタントはデフォルトではインストールされません．必要に応じて個別または一括でインストールしてください．

| ツール | コマンド | 個別インストール |
|---|---|---|
| GitHub Copilot CLI | `copilot` | `bash .devcontainer/install-copilot.sh` |
| OpenCode | `opencode` | `bash .devcontainer/install-opencode.sh` |
| Codex | `codex` | `bash .devcontainer/install-codex.sh` |
| Claude Code | `claude` | `bash .devcontainer/install-claude.sh` |
| Ollama | `ollama` | `bash .devcontainer/install-ollama.sh` |

全てを一括でインストールするには：
```bash
bash .devcontainer/install-all-agents.sh
```

とりあえず使ってみるだけであればGithub Educationに登録してGitHub Copilot CLIかOpenCodeを使うのがお手軽かと思います．

### LaTeX（日本語）のインストール（任意）
この環境でLaTeX（日本語）文書を編集する場合には，以下のコマンドで必要なパッケージをインストールしてください．  
（ただし2GB程度のストレージを消費しますので使用制限に注意してください．）
...
このスクリプトでは `texlive-latex-recommended`，`texlive-lang-japanese`，`latexmk` をインストールします．
